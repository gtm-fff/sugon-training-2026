...
一个本机可运行的前后端一体 Demo：访客免登录上传一份不超过 25MB 的照片或视频，上传后自动生成代码，并可凭代码回查、修改；管理员可统一查看、编辑、删除和按连队导出 ZIP。连队支持一连至十六连，最新真实投稿会优先成为相册封面，点击后可浏览该连队全部内容。

图片上传时由浏览器额外生成不超过 2MB 的 WebP 展示图和最长边 720px 的 WebP 缩略图，三者同时保存到 R2。相册放大浏览使用展示图，列表使用缩略图，管理员导出仍使用原图；历史投稿没有派生图片时会自动回退原图。视频不做浏览器转码，限制为 25MB。
